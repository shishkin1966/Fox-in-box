...
class GetMaterialsListRequest(subscribe: String, private val skip: Int, private val take: Int) :
    AbsNetResultMessageRequest(subscribe) {
    companion object {
        const val NAME = "GetMaterialsListRequest"
    }

    override fun getData(): Single<BaseResponse>? {
        return ApplicationSingleton.instance.getApi().getMaterialsList(skip, take)
    }
